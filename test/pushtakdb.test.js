...
var { initiateDB } = require('../pushtakdb');
    ...
    describe('db', function () {
        ...
        it('it should create a db instance', function (done) {
            ...
            synchronize(
                (next) => {
                    fs.readFile(
                        path.join(dbpath, dbname, 'nodb.json'),
                        'utf-8',
                        (e, d) => {
                            next(JSON.parse(d));
                        }
        ...
        it('it should add collection', function (done) {
            ...
            expect(fs.existsSync(path.join(dbpath, dbname, 'testcol.json'))).to
                .be.true;
            synchronize(
                (next) => {
                    fs.readFile(
                        path.join(dbpath, dbname, 'testcol.json'),
                        'utf-8',
                        (e, d) => {
                            next(JSON.parse(d));
                        }
        ...
        it('it should create collection with config', function (done) {
            ...
            synchronize(
                (next) => {
                    fs.readFile(
                        path.join(dbpath, dbname, 'testcol_config.json'),
                        'utf-8',
                        (e, d) => {
                            next(JSON.parse(d));
                        }
        ...
        it('it should add date audit fields with insert', function (done) {
            let db = initiateDB(dbpath, dbname);
            db.collection('testcol_config').insert(
                { name: 'ashutosh' },
                (record) => {
        ...
        it('it should able to find inserted records', function (done) {
            ...
            synchronize(
                (next) =>
                    db
                        .collection('name_test')
                        .insert({ name: 'ashutosh' }, (r) => next()),
                (next) =>
                    db
                        .collection('name_test')
                        .insert({ name: 'mahamaya' }, (r) => next()),
                (next) => {
        ...
        it('it should update filtered record', function (done) {
            ...
            synchronize(
                (next) => {
                    db.collection('name_test').update(
                        { name: 'ashutosh' },
                        { name: 'new ashutosh' },
                        (r) => {
            ...
            synchronize(
                ...
                (next) =>
                    db
                        ...
                        .insert({ name: 'mahamaya' }, (r) => next()),
                (next) => {
                    db.collection('name_test2').delete(
                        { name: 'ashutosh' },
                        (r) => {
                            expect(r.count).to.equal(1);
                            next();
        ...
        it('it should hard delete filtered records', function (done) {
            ...
            synchronize(
                (next) => {
                    db.collection('name_test2').hardDelete(
                        { name: 'mahamaya' },
                        (r) => {
                            expect(r.count).to.equal(1);
                            next();
